...
const stopBackgroundMusicButton = document.getElementById("stop-background-music");
...
stopBackgroundMusicButton.addEventListener("click", backgroundThemeMusic);

function backgroundThemeMusic() {
    if (bmbClick % 2 === 0) {
        backgroundMusic.play();
    } else {
...
spinButton.addEventListener('click', () => {
    ...
    reels.forEach((reel) => {
        ...
        symbolsInReel.forEach((symbol) => {
            ...
            symbol.classList.remove('slot_win');
...
function fillMiddleRow() {
    reels.forEach((reel) => {
        ...
        symbolsInMiddleReel.classList.add('slot_win');
    });
}